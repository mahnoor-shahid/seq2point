...
class Seq2PointDataLoader():
    ...
    def __init__(self):
        try:
            self.__target_appliance = target_appliance=TRAINING_CONFIG['TARGET_APPLIANCE']
            self.__target_houses = TRAINING_CONFIG['TARGET_HOUSES']
            self.__subset_days = TRAINING_CONFIG['SUBSET_DAYS']
            self.__normalize_with = TRAINING_CONFIG['NORMALIZE']
            self.__proportion = TRAINING_CONFIG['SPLIT_PROPORTION']
            ...
            if bool(self.__normalize_with)==True:
                self.__appliance_obj.normalize(scaler=self.__normalize_with, scalars_directory=os.path.join(TRAINING_CONFIG['EXPERIMENT_PATH'], 'scalers'), training = True)
